/datum/transport_controller/linear/tram
	...
	///how many times we moved while costing more than SSicts_processing.max_time milliseconds per movement.
	///if this exceeds SSicts_processing.max_exceeding_moves
	var/recovery_activate_count = 0

	///how many times we moved while costing less than 0.5 * SSicts_processing.max_time milliseconds per movement
...
/datum/transport_controller/linear/tram/proc/check_starting_landmark()
	...
	SSicts_processing.can_fire = TRUE
 ...
 * literally ripping itself apart. all of the actual movement is handled by SSicts_processing
...
/datum/transport_controller/linear/tram/proc/dispatch_tram(obj/effect/landmark/icts/nav_beacon/tram/destination_platform)
	...
	START_PROCESSING(SSicts_processing, src)
...
/datum/transport_controller/linear/tram/process(seconds_per_tick)
	if(!travel_remaining)
		...
		if(recovery_mode)
			if(duration <= (SSicts_processing.max_time / 2))
				recovery_clear_count++
			else
				recovery_clear_count = 0

			if(recovery_clear_count >= SSicts_processing.max_cheap_moves)
				speed_limiter = base_speed_limiter
				recovery_mode = FALSE
				recovery_clear_count = 0

		else if(duration > SSicts_processing.max_time)
			recovery_activate_count++

			if(recovery_activate_count >= SSicts_processing.max_exceeding_moves)
				message_admins("The tram at [ADMIN_JMP(transport_modules[1])] is taking more than [SSicts_processing.max_time] milliseconds per movement, halving its movement speed. if this continues to be a problem you can call reset_lift_contents() on the trams lift_master_datum to reset it to its original state and clear added objects")
...
/datum/transport_controller/linear/tram/proc/update_tram_doors(action)
	...
	for(var/obj/machinery/door/window/tram/tram_door in GLOB.airlocks)
		if(tram_door.associated_lift != specific_transport_id)
			continue
